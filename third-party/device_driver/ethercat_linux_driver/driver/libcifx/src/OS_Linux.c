 ...
 **************************************************************************************
   ...
   Changes:
     ...
     ----------------------------------------------------------------------------------
     15       13.07.2015   FM           Bugfix:
                                         - netx_irq_thread()/OS_EnableInterrupts()/OS_DisableInterrupts()
                                           DPM accesses were implemented without the HWIF functions.
                                        Changed:
                                         - netx_irq_thread()
                                           Now, the dummy read is tested against read failure instead of the expected amount of read bytes.
     14       05.05.2014   SD           Changed:
                                         - OS_WaitMutex()/OS_WaitEvent():
                                           Incorrect timeout calculation aborts blocking to early.
     13       30.04.2013   SD           Changed:
                                         - if NO_MIN_SLEEP is defined, if OS_Sleep is called with 0, the function returns
                                           immediately
     13       04.02.2013   SD           Changed:
                                         - Bugfix: IRQ-thread priority settings are ignored since PTHREAD_EXPLICIT_SCHED is not set
     12       16.01.2013   SD           Changed:
                                         - fixed OS_Memcpy() (due to alignment errors on 64-bit platform)
     11       14.09.2012   SD           Changed:
                                         - changed OS_Memcpy() for aligned access
     10       16.01.2012   SD           Added:
                                         - OS_Time()
     9        17.11.2011   SD           Changed:
                                         - removed perrro() after pthread-API calls (replaced by printing error val to sdterr,
                                           because pthread functions do not set errno)
                                         - OS_Sleep() keep on sleeping, when interrupted by a system signal
                                         - OS_WaitMutex(): removed error message when timed out
     8        30.09.2009   MT           Changed:
                                         - Minumum Stack size adjusted, so it should work with system
                                           that really set PTHREAD_STACK_MIN to lowest possible values
     7        16.06.2009   RM           Bugfix:
                                         - OS_EnableInterrupts() /  OS_DisableInterrupts()
                                           missing swap macro during write of IRQ_ENABLE_MASK included
     6        02.04.2009   MT           Bugfix:
                                         - Events were using CLOCK_REALTIME instead of CLOCK_MONOTONIC. This may result in
                                           invalid waiting times, if the system times gets adjusted during wait.
     5        19.03.2009   MT           Bugfix:
                                         - OS_SetEvent did not signal OS_WaitEvent under certain conditions,
                                           disallowing interrupts to work properly
     4        04.03.2009   MT           Bugfix:
                                         - IRQ re-enabling from netx_irq_thread did not work on
                                           big endian systems
     3        16.02.2009   MT           Added
                                         - IRQ thread priority can now be set up through device.conf
                                        Changed:
                                         - IRQ thread modified to use minimum stack size (PTHREAD_STACK_MIN)
...
#ifndef CIFX_TOOLKIT_DISABLEPCI
  if(0 != pci_system_init())
  {
    perror("Error initializing PCI access subsystem");
...
static void *netx_irq_thread(void *ptr) {
  ...
  while( info->irq_stop == 0 )
  {
    ...
    sel_timeout.tv_usec = 500 * 1000; /* Default wait timeout = 500ms */

    if(1 == select(nfds, &readfd, NULL, NULL, &sel_timeout))
    {
      if(read( info->userdevice->uio_fd, &ulVal, sizeof(ulVal) ) < 0)
        perror("IRQ read");
#ifdef VERBOSE_1
      printf("IRQ @status_thread\n");
#endif
      isr_ret = cifXTKitISRHandler(info->devinstance, 1);
...
void OS_EnableInterrupts(void* pvOSDependent) {
  ...
  if(info->set_irq_scheduler_algo)
  {
    ...
    if(pthread_attr_setschedpolicy(&info->irq_thread_attr, info->irq_scheduler_algo) != 0)
    {
      perror("Error setting custom thread scheduling algorithm for IRQ thread");
    }
  }
  ...
  if(info->set_irq_prio)
  {
    ...
    pthread_attr_setinheritsched( &info->irq_thread_attr, PTHREAD_EXPLICIT_SCHED);
    if(pthread_attr_setschedparam(&info->irq_thread_attr, &sched_param) != 0)
    {
      perror("Error setting custom thread priority in IRQ thread");
    }
  }

  if( pthread_create( &info->irq_thread, &info->irq_thread_attr, netx_irq_thread,
                      (void*)info ) != 0 )
  {
    perror("Enabling Interrupts");
  } else
  {
    info->irq_stop = 0;
    if(info->devinstance->ulDPMSize >= NETX_DPM_MEMORY_SIZE) {
...
int add_msec_to_timespec( struct timespec* time_val, uint32_t msec)
{
  ...
  time_val->tv_sec += msec / 1000;
  msec             -=  msec / 1000;
  msec              = msec * 1000 * 1000;

  if ((msec + time_val->tv_nsec) >= 1000000000)
  {
    time_val->tv_sec++;
    time_val->tv_nsec = (msec + time_val->tv_nsec) - 1000000000;
  } else
  {
    time_val->tv_nsec += msec;
  }
  return 0;
}
