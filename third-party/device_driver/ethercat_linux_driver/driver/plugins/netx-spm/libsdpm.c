 ...
 *******************************************************************************
 ...
 Description:
 This library can be used to initialize and use a spidev as a cifX device.
 ...
 2        15.12.2015   FM       - bugfix:SDPMInit():
                                  An initialization of invalid SPI devices results in a segmentation fault.
...
static void SPIReadChunk( struct SPI_PARAM_T* ptSPIParam, uint32_t ulDpmAddr, uint8_t *pbData, uint32_t ulLen)
{
  ...
  ret = SPITransferMessage(ptSPIParam, (uint8_t*)ptSPIRDMsg, ulLen+4, 0);
#ifdef CHECK_STATE
  if (0>ret) {
...
static void SPIWriteChunk(struct SPI_PARAM_T* ptSPIParam, uint32_t ulDpmAddr, uint8_t *pbData, uint32_t ulLen)
{
  ...
  /* transfer message */
  ret = SPITransferMessage(ptSPIParam, (uint8_t*)ptSPIWRMsg, ulLen+3, 0);
#ifdef CHECK_STATE
  if (0>ret) {
...
static void* SPIHWIFRead_ext(struct CIFX_DEVICE_T* ptDevice, void* pvDpmAddr, void* pvDst, uint32_t ulLen)
{
  ...
  while(reads>0) {
    ...
    pvDpmAddr = (void*)((uint32_t)pvDpmAddr + ptSPIParam->ulChunkSize);
    pvDst     = (void*)((uint32_t)pvDst     + ptSPIParam->ulChunkSize);
    reads--;
  }
...
static void* SPIHWIFWrite_ext(struct CIFX_DEVICE_T* ptDevice, void* pvDpmAddr, void* pvSrc, uint32_t ulLen)
{
  ...
  while(writes>0) {
    ...
    pvDpmAddr = (void*)((uint32_t)pvDpmAddr + ptSPIParam->ulChunkSize);
    pvSrc     = (void*)((uint32_t)pvSrc     + ptSPIParam->ulChunkSize);
    writes--;
  }
  if (ulLen%ptSPIParam->ulChunkSize)
...
struct CIFX_DEVICE_T* SDPMInit(uint8_t *pszSPIDevice, uint8_t bMode, uint8_t bBits, uint32_t ulFrequency, uint8_t *pszIRQFile, uint32_t ulChunkSize)
{
  ...
  uint8_t szSPIDevice[256];
  int32_t lFd;

  DBG("++SDPMInit\n");

  /* Allocate memory for the cifX device */
  ptSPIDev = malloc(sizeof(*ptSPIDev));
  if(ptSPIDev == NULL) {
    ...
    if(NULL != dir) {
      while((entry = readdir(dir)) != NULL) {
        if((0 == strncmp(entry->d_name, "spidev", 6))) {
          sprintf(szSPIDevice,"/dev/%s", entry->d_name);
  ...
  if((bMode < SPI_MODE_0) || (SPI_MODE_3 < bMode)) {
    fprintf(stderr, "SDPMInit: Invalid SPI mode (%d <= x <= %d)\n", SPI_MODE_0, SPI_MODE_3);
    goto error_out;
  }

  /* Check for a valid SPI frequency */
  if((ulFrequency < 50000) || (25000000 < ulFrequency)) {
    fprintf(stderr, "SDPMInit: Invalid SPI frequency (50000 <= x <= 25000000)\n");
    goto error_out;
  }

  /* Check for a valid IRQ file (optional) */
  if(pszIRQFile != NULL) {
    lFd = open(pszIRQFile, O_RDONLY);
    if(lFd < 0) {
      fprintf(stderr, "SDPMInit: Invalid SPI IRQ file (%s)\n", pszIRQFile);
      goto error_out;
    }
  }
  else {
    lFd = -1; /* Fallback to polling mode */
  }
