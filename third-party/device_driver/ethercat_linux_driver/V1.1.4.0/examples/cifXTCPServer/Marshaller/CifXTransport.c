 ...
 **************************************************************************************
   ...
   Description:
    Imlpementation of Marshaller function call interpretation
...
*   \return !=0 on success                                                   */
/*****************************************************************************/
TLR_RESULT cifXTransportInit(void* pvMarshaller, void* pvConfig)
{
  ...
  } else
  {
    ...
    memset(ptInstance, 0, sizeof(*ptInstance));
        ...
        (CIFX_NO_ERROR == ptInstance->tDRVFunctions.pfnxDriverGetInformation(ptInstance->hDriver, sizeof(tDriverInfo), &tDriverInfo)) )
    {
      if(0 != (ptInstance->ptDevices = OS_MALLOC((uint32_t)sizeof(*ptInstance->ptDevices) * tDriverInfo.ulBoardCnt)))
      {
        ...
        memset(ptInstance->ptDevices, 0, sizeof(*ptInstance->ptDevices) * tDriverInfo.ulBoardCnt);
        ...
        for(ulBoard = 0; ulBoard < tDriverInfo.ulBoardCnt; ulBoard++)
        {
          if(CIFX_NO_ERROR == ptInstance->tDRVFunctions.pfnxDriverEnumBoards(ptInstance->hDriver, ulBoard, sizeof(ptDevice->tBoardInfo), &ptDevice->tBoardInfo))
          {
            ...
            memset(ptDevice->ptChannels, 0, sizeof(*ptDevice->ptChannels) * ptDevice->ulChannelCount);
    ...
    {
      eRet = (TLR_RESULT)CIFX_DRV_DRIVER_NOT_LOADED;
    }
  ...
  case MARSHALLER_DRV_METHODID_GETINFO:
    {
      ...
      } else
      {
        ...
        if(CIFX_NO_ERROR == lRet)
        {
          ...
          {
            memset(&ptReq->tData, 0, sizeof(tDriverInfo));
            memcpy(&ptReq->tData, &tDriverInfo, ulCopyLen);
  ...
  case MARSHALLER_DRV_METHODID_ENUMBOARDS:
    {
      ...
      } else
      {
        ...
        if(lRet == CIFX_NO_ERROR)
        {
          ...
          if( (ptBuffer->tMgmt.ulDataBufferLen - sizeof(MARSHALLER_DATA_FRAME_HEADER_T)) >= ulCopyLen )
          {
            memset(&ptEnumReq->tData, 0, sizeof(tBoardInfo));
            memcpy(&ptEnumReq->tData, &tBoardInfo, ulCopyLen);
  ...
  case MARSHALLER_DRV_METHODID_ENUMCHANNELS:
    {
      ...
      } else
      {
        ...
        if(lRet == CIFX_NO_ERROR)
        {
          ...
          if( (ptBuffer->tMgmt.ulDataBufferLen - sizeof(MARSHALLER_DATA_FRAME_HEADER_T)) >= ulCopyLen )
          {
            memcpy(&ptEnumReq->tData, &tChannelInfo, ulCopyLen);
            ptEnumReq->tHeader.ulDataSize = ulCopyLen;
          }else
          {
            lRet = CIFX_BUFFER_TOO_SHORT;
  ...
  case MARSHALLER_SYSDEV_METHODID_PUTPACKET:
    {
      ...
      } else
      {
        ...
        memcpy(&ulSendSize, pbRequest, sizeof(ulSendSize));
        memcpy(&ulTimeout,  &pbRequest[sizeof(ulSendSize) + ulSendSize], sizeof(ulTimeout));
  ...
  case MARSHALLER_SYSDEV_METHODID_FINDFIRSTFILE:
    {
      ...
      } else
      {
        ...
        *((uint32_t*)&tDirEntry.hList) = ptFindReq->tData.hList;
        ...
        memcpy(tDirEntry.szFilename, ptFindReq->tData.szFilename, sizeof(ptFindReq->tData.szFilename));
        ...
        if(CIFX_NO_ERROR == lRet)
        {
          ...
          memcpy(ptFindCnf->tData.szFilename, tDirEntry.szFilename, sizeof(ptFindCnf->tData.szFilename));
  ...
  case MARSHALLER_CHANNEL_METHODID_GETSENDPACKET:
    {
      ...
      } else
      {
        ...
        memcpy(&ulSize, pbRequest, sizeof(ulSize));
