...
class Rocket_Flight_DM : public Dynamics {
 ...
 private:
  ...
  arma::mat TBI; /* *io (--)    Transformation Matrix of body coord wrt inertia
                    coord */
  double _TBI[3][3]; /* *io (--)    Transformation Matrix of body coord wrt
                        inertia coord */

  arma::mat TBID; /* *io (--)    Transformation Matrix of body coord wrt inertia
                     coord derivative */
  double _TBID[3][3]; /* *io (--)    Transformation Matrix of body coord wrt
                         inertia coord derivative */

  arma::vec TBI_Q;  /* *io (--)    Transformation Matrix of body coord wrt
                       inertia coord (Quaternion) */
  double _TBI_Q[4]; /* *io (--)    Transformation Matrix of body coord wrt
                       inertia coord (Quaternion) */

  arma::vec TBID_Q;  /* *io (--)    Transformation Matrix of body coord wrt
                        inertia coord derivative (Quaternion) */
  double _TBID_Q[4]; /* *io (--)    Transformation Matrix of body coord wrt
                        inertia coord derivative (Quaternion) */

  arma::mat TBD; /* *io (--)    Transformation Matrix of body coord wrt geodetic
                    coord */
  double _TBD[3][3]; /* *io (--)    Transformation Matrix of body coord wrt
                        geodetic coord */
  ...
  double _WEII_skew[3]
                   [3]; /* **  (r/s)    Earth's angular velocity (skew-sym) */
  ...
  arma::vec
      ABIB; /* *o  (m/s2)   Vehicle inertia acceleration on body coordinate */
  double _ABIB[3]; /* *o  (m/s2)   Vehicle inertia acceleration on body
                      coordinate */
  ...
  arma::mat TDI; /* **  (--)     Transformation Matrix of geodetic wrt inertial
                    coordinates */
  double _TDI[3][3]; /* **  (--)     Transformation Matrix of geodetic wrt
                        inertial  coordinates */

  arma::mat
      TGI; /* **  (--)     Transformation Matrix geocentric wrt inertia coord */
  double _TGI[3][3]; /* **  (--)     Transformation Matrix geocentric wrt
                        inertia coord */
  ...
  arma::vec WBII;  /* *io (r/s)        Vehicle's inertia angular velocity in
                      inertia coord */
  double _WBII[3]; /* *io (r/s)        Vehicle's inertia angular velocity in
                      inertia coord */

  arma::vec WBEB;  /* *io (r/s)        Angular velocity of vehicle wrt earth in
                      body coord */
  double _WBEB[3]; /* *io (r/s)        Angular velocity of vehicle wrt earth in
                      body coord */

  arma::vec WBIB; /* *io (r/s)        Augular velocity of vehicle wrt inertia in
                     body coord */
  double _WBIB[3]; /* *io (r/s)        Augular velocity of vehicle wrt inertia
                      in body coord */

  arma::vec WBIBD;  /* *io (r/s2)       Angular velocity of vehicle wrt inertia
                       in body coord - derivative */
  double _WBIBD[3]; /* *io (r/s2)       Angular velocity of vehicle wrt inertia
                       in body coord - derivative */

  arma::vec TVD; /* **  (--)     Transformation Matrix of geographic velocity
                    wrt geodetic coord */
  double _TVD[3][3]; /* **  (--)    Transformation Matrix of geographic velocity
                        wrt geodetic coord */
  ...
  /* Generating Outputs */
  double
      ortho_error; /* *io (--)    Direction cosine matrix orthogonality error*/
  ...
  double ppx; /* *io (d/s)        Body roll angular velocity wrt earth in body
                 axes */
  double qqx; /* *io (d/s)        Body pitch angular velocity wrt earth in body
                 axes */
  double rrx; /* *io (d/s)        Body yaw angular velocity wrt earth in body
                 axes */
  double control_loss;

  /* Orbital Logging */
  double
      _inclination;     /* *o  (deg)   [DIAG] Orbital inclination is the minimun
  ...
  double _arg_perix;  /* *o  (deg)   [DIAG] The argument of periapsis (also
                         ...
                         direction of motion */
  double
      _true_anomx; /* *o  (deg)   [DIAG] In celestial mechanics, true anomaly is
