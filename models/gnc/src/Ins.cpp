...
void INS::set_liftoff(unsigned int index) {
    this->liftoff = index;
}
...
void INS::calculate_INS_derived_TEI() {
    ...
    UTC = utc_caldate.get_hour() * 3600.0 + utc_caldate.get_min() * 60.0 + utc_caldate.get_sec();

    index = static_cast<int>(time->get_modified_julian_date().get_mjd() - 55197.00);   /* get dUT1 = Ut1 - UT from table*/
    ...
    /*-------------- Precession Matrix  ------------------- */
    /*----------------------------------------------------------- */
    t = (time->get_modified_julian_date().get_jd() - 2451545.0) / 36525.0;  /* J2000.5 : Julian Day is 2451545, unit in day */
...
void INS::calculate_INS_derived_euler_angles(arma::mat33 TBD) {
...
void INS::update(double int_step) {
    ...
    if (this->liftoff == 0) TBICI = TLI * TDCI;

    // computing geodetic velocity from INS
    arma::vec3 VBECD = TDCI * trans(TEIC) * VBEEC;
...
arma::vec3 INS::euler_angle(arma::mat33 TBD) {
    double psibdc(0), thtbdc(0), phibdc(0);
    double cthtbd(0);
...
*
********************************************************************************/
arma::vec INS::AccelHarmonic(arma::vec3 SBII, double CS[21][21], int n_max, int m_max, arma::mat33 TEIC) {
    /* Local variables */
    ...
    ********************************/
    ...
    arma::vec tmp_vec = trans(TEIC) * a_bf;
    return tmp_vec;
